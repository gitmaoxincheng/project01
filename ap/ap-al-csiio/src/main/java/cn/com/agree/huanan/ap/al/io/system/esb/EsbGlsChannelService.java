 ...
 * json报文
 */
public class EsbGlsChannelService extends IOService {
	...
	public static class AppHeaderI extends MsgHeader {
		...
		private  MsgSegment init() {
			...
			headStruct.addNode(new FieldNode("prcscd", new MsgField(ContentEnum.MessageType.STRING.toString(), "prcscd", 20, 0, true, "交易码")));//txn_code由 原子服务表记录
			messageNode.addStructNode(headStruct);
			return messageNode;
		}
	...
	public void initContent(Map<String, Object> tradeContext) {
		...
		appHeader.put("prcscd", tradeContext.get("AtExtCode"));
	}
