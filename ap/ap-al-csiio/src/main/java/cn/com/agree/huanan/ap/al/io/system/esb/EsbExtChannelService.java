 ...
 * json报文
 */
public class EsbExtChannelService extends IOService {
...
/*			esbheadStruct.addNode(new FieldNode("SrcIP1", new MsgField(ContentEnum.MessageType.STRING.toString(), "SrcIP1", 128, 0, false, "源请求方服务器IP")));  // ESB系统虽定义，但未使用，需严格按照接口定义上传
	...
	public static class AppHeaderI extends MsgHeader {
		...
		private  MsgSegment init() {
			...
			headStruct.addNode(new FieldNode("AuthPw", new MsgField(ContentEnum.MessageType.STRING.toString(), "AuthPw", 20, 0, true, "验证密码")));
			headStruct.addNode(new FieldNode("dataQueryType", new MsgField(ContentEnum.MessageType.STRING.toString(), "dataQueryType", 10, 0, true, "请求类型")));
			headStruct.addNode(new FieldNode("dataQueryDays", new MsgField(ContentEnum.MessageType.STRING.toString(), "dataQueryDays", 10, 0, true, "请求天数")));
			headStruct.addNode(new FieldNode("resendFlag", new MsgField(ContentEnum.MessageType.STRING.toString(), "resendFlag", 2, 0, false, "重发标识")));
			headStruct.addNode(new FieldNode("lastGloSeqNo", new MsgField(ContentEnum.MessageType.STRING.toString(), "lastGloSeqNo", 25, 0, false, "原全局流水号")));
			headStruct.addNode(new FieldNode("businessno", new MsgField(ContentEnum.MessageType.STRING.toString(), "businessno", 50, 0, false, "业务流水号	")));
			messageNode.addStructNode(headStruct);
			return messageNode;
		}
		/* （非 Javadoc）
...
/*	@Override
	public IOService buildOutMessageContext(Map<String, Object> tradeContext) {
		errorCode = new ThreadLocal<String>();
		errorMsg = new ThreadLocal<String>();
		getErrorCode();	//特殊处理，因为映射前后ErrorCode ErrorMsg 位置不同了,与super.buildOutMessageContext(tradeContext)顺序不可调换
		getErrorMessage();
		super.buildOutMessageContext(tradeContext);
		return this;
	}	*/
}
