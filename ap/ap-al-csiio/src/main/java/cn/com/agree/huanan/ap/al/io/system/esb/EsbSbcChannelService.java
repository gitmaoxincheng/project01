 ...
 * json报文  
 */ 
public class EsbSbcChannelService extends IOService {
...
/*			esbheadStruct.addNode(new FieldNode("SrcIP1", new MsgField(ContentEnum.MessageType.STRING.toString(), "SrcIP1", 128, 0, false, "源请求方服务器IP")));  // ESB系统虽定义，但未使用，需严格按照接口定义上传
	...
	public static class EsbHeaderO extends MsgHeader {
		...
		/**
		 * 互联网金融平台返回内容特殊处理， 因为ESB系统没有强执行 RspStatus的规范，互联网金融平台返回没有包含RspStauts
		 * 但如果是ESB本身的错误，会包含RspStutus，暂定将RspStatus设为非必输。
		 */
	...
	public static class AppHeaderI extends MsgHeader {
		...
		private  MsgSegment init() {
			...
			headStruct.addNode(new FieldNode("TellerName", new MsgField(ContentEnum.MessageType.STRING.toString(), "TellerName", 20, 0, false, "柜员名称")))
					  .addNode(new FieldNode("IdtfTp", new MsgField(ContentEnum.MessageType.STRING.toString(), "IdtfTp", 2, 0, false, "	企业证件类型")))
					  .addNode(new FieldNode("IdtfNo", new MsgField(ContentEnum.MessageType.STRING.toString(), "IdtfNo", 20, 0, false, "企业证件号码")))
					  .addNode(new FieldNode("name", new MsgField(ContentEnum.MessageType.STRING.toString(), "name", 120, 0, false, "企业客户名称")))
					  ;
			messageNode.addStructNode(headStruct);
			return messageNode;
		}
