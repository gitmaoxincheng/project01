...
public class B_ServiceInvoker3 {
	...
	public static <T, R> IFuture getIFuture3(IFuture future,String src, String target,Class<T> rspType, Function<JavaDict, JavaDict> action) {
		// 创建适配器
		return new AbstractIFutureAdapter() {
			...
			public synchronized Object get() throws InterruptedException, ExecutionException {
				...
				try {
					logger.debug("MTEST1");
					FileUtil.writeData2File("MTEST1\n", "/export/123.txt", true, "UTF-8");
					FileUtil.writeData2File(Thread.currentThread().getName()+"\n", "/export/123.txt", true, "UTF-8");
					// 获取结果
					Object rspObj = future.get();
					logger.debug("MTEST2");
					logger.debug("TTTTTTTTTEEE");
					logger.debug("future type:%s",future.getClass().getName());
					logger.debug("rspObj.class:%s",rspObj.getClass().getName());
					logger.debug(rspObj.toString());
					FileUtil.writeData2File("MTEST2\n", "/export/123.txt", true, "UTF-8");
					FileUtil.writeData2File(Thread.currentThread().getName()+"\n", "/export/123.txt", true, "UTF-8");
					// 出现异常
					//					if (futureEx != null) {
					// 判断是否失败
					...
					// 处理结果：T -> byte[]
					JavaDict msgDict = action.apply((JavaDict) rspObj);
					logger.debug("MTEST3");
					FileUtil.writeData2File("MTEST3\n", "/export/123.txt", true, "UTF-8");
					FileUtil.writeData2File(Thread.currentThread().getName()+"\n", "/export/123.txt", true, "UTF-8");
					// 返回
					// 通讯成功，处理业务结果
					...
					throw new ApEumnNotExistException("111");
					...
					//						apEx = new ServiceInvokeTimeOutlException("111", "222");
					//					} else {
					//						apEx = new ServiceInvokeFailException("111", "222");// 服务调用异常
					//					}
					//					// 返回
					//					return isFailed
			...
			public IFuture addListener(IFutureListener listener) {
				FileUtil.writeData2File("MTEST4\n", "/export/123.txt", true, "UTF-8");
				FileUtil.writeData2File(Thread.currentThread().getName()+"\n", "/export/123.txt", true, "UTF-8");
				// 适配
				// 保存结果
				// 通知
				try {
					//						logger.debug("Mark8");
					FileUtil.writeData2File("MTEST5\n", "/export/123.txt", true, "UTF-8");
					FileUtil.writeData2File(Thread.currentThread().getName()+"\n", "/export/123.txt", true, "UTF-8");
					listener.operationCompleted(this);
					//						logger.debug("Mark9");
					FileUtil.writeData2File("MTEST6\n", "/export/123.txt", true, "UTF-8");
					FileUtil.writeData2File(Thread.currentThread().getName()+"\n", "/export/123.txt", true, "UTF-8");
