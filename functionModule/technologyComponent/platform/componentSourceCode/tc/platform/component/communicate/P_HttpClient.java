...
public class P_HttpClient {
	...
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "发送HTTP Post请求", style = "判断型", type = "同步组件", date = "Mon Jul 27 12:39:11 CST 2015")
	public static TCResult doPost(String url, JavaDict header,
			String requestBody, int connectTimeout, int socketTimeout, String encoding) {
		if (url == null) {
		...
		try {
			...
			httpPost.setEntity(new ByteArrayEntity(requestBody.getBytes(encoding)));
			if (header != null) {
			...
			String responseBody = client.execute(httpPost, new Handler(encoding));
			return TCResult.newSuccessResult(responseBody.getBytes(encoding));
	...
	@Returns(returns = { @Return(id = "0", desp = "失败"),
			@Return(id = "1", desp = "成功") })
	@Component(label = "发送HTTP Get请求", style = "判断型", type = "同步组件", date = "Mon Jul 27 13:20:46 CST 2015")
	public static TCResult doGet(String url, JavaDict header, JavaDict params,
		...
		try {
			if(params != null) {
				Iterator<Entry<Object, Object>> paramsIterator = params.entrySet().iterator();
				while(paramsIterator.hasNext()) {
					Map.Entry<Object, Object> entry = paramsIterator.next();
					String key = (String) entry.getKey();
					String value = (String) entry.getValue();
					paramStr += "&" + key + "=" + URLEncoder.encode(value, encoding);
				}
			}
		...
		try {
			...
			if(header != null) {
				Iterator<Entry<Object, Object>> headIterator = header.entrySet().iterator();
			...
			String responseBody = client.execute(httpget, new Handler(encoding));
	...
	static class Handler implements ResponseHandler<String> {
		
		private String encoding;
		
		public Handler(String encoding) {
